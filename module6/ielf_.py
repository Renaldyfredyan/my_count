...
class iEFLModule(nn.Module):
    def __init__(
        ...
        self.exemplar_attention = nn.MultiheadAttention(
            emb_dim, num_heads, dropout, batch_first=True
        )
    ...
    def forward(self, f_e, pos_emb, bboxes):
        ...
        for k in range(self.num_iterative_steps):
            ...
            F_k = F_k.reshape(-1, self.emb_dim).contiguous()  # [N, E]
            memory = memory.reshape(-1, memory.size(-1)).contiguous()  # [M, E]
            F_k = self.norm2(F_k + self.linear_attention(
                query=F_k,
                key_value=memory
            ))
            F_k = F_k.reshape(-1, bs, self.emb_dim)  # kembalikan ke bentuk asli
...
class LinearAttention(nn.Module):
    ...
    def forward(self, query, key_value):
        # query shape: [N, E] where N is sequence length
        # key_value shape: [M, E] where M is memory sequence length
        
        # Project and scale
        q = self.q_proj(query) * self.scale  # [N, E]
        k = self.k_proj(key_value)  # [M, E]
        v = self.v_proj(key_value)  # [M, E]
        
        # Apply row-wise softmax
        q = q.softmax(dim=-1).contiguous()  # [N, E]
        k = k.softmax(dim=-2).contiguous()  # [M, E]
        
        # Linear attention computation
        context = torch.matmul(k.transpose(-2, -1), v)  # [E, E]
        out = torch.matmul(q, context)  # [N, E]
