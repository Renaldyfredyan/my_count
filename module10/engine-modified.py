...
class efficient(nn.Module):
    ...
    def __init__(
        ...
        # Backbone - always needed but can be frozen/unfrozen
        self.backbone = SwinBackbone(
            reduction=reduction,
            requires_grad=train_backbone
        )

        # Hybrid Encoder - optional (can be skipped if num_encoder_layers = 0)
        if num_encoder_layers > 0:
            ...
            # Simple projection if no encoder is used
            self.input_proj = nn.Conv2d(self.backbone.total_channels, emb_dim, kernel_size=1)

        # iEFL Module - always needed but can vary in steps
        ...
        # Regression head and auxiliary heads
        self.regression_head = DensityMapRegressor(emb_dim, reduction)
        
        # Aux heads are optional (tied to iefl steps)
        if num_iefl_iterative_steps > 1:
            self.aux_heads = nn.ModuleList([
                DensityMapRegressor(emb_dim, reduction)
                for _ in range(num_iefl_iterative_steps - 1)
            ])
        else:
            # Create empty ModuleList for consistent interface
            self.aux_heads = nn.ModuleList()

        # Positional encoding - always needed
        self.pos_emb = PositionalEncodingsFixed(emb_dim)

    def forward(self, x, bboxes):
        num_objects = bboxes.size(1) if not self.zero_shot else self.num_objects
        
        # Get backbone features (always needed)
        s3, s4, s5 = self.backbone.forward_multiscale(x)
        
        # Process through encoder if available, otherwise use simple projection
        ...
        # Process each prototype and generate output/auxiliary outputs
        for i in range(all_prototypes.size(0)):
            ...
            # Use regression head for final output, aux heads for intermediate steps
            if i == all_prototypes.size(0) - 1:
                predicted_dmaps = self.regression_head(response_maps)
            elif i < len(self.aux_heads):
                predicted_dmaps = self.aux_heads[i](response_maps)
            else:
                # If we don't have enough aux heads, use main head
                predicted_dmaps = self.regression_head(response_maps)
                
            outputs.append(predicted_dmaps)

        # Return final output and auxiliary outputs (if any)
        return outputs[-1], outputs[:-1] if len(outputs) > 1 else []
