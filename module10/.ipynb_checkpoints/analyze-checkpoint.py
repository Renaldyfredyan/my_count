...
def analyze_counting_errors(args):
    ...
    for split in ['val', 'test']:
        ...
        # Exclusive categorization (exact, over, under)
        over_count = 0
        under_count = 0
        exact_count = 0

        for p, g in zip(filtered_pred_counts, filtered_gt_counts):
            if round(p) == round(g):
                exact_count += 1
            elif p > g:
                over_count += 1
        ...
        print("\nTop 10 worst predictions (excluding outliers):")
        for img_name, gt, pred, error, rel_error in sorted_errors[:10]:
