...
def analyze_counting_errors(args):
    ...
    for split in ['val', 'test']:
        ...
        # Calculate over/under counting statistics
        over_count = sum(1 for p, g in zip(all_pred_counts, all_gt_counts) if p > g)
        under_count = sum(1 for p, g in zip(all_pred_counts, all_gt_counts) if p < g)
        exact_count = sum(1 for p, g in zip(all_pred_counts, all_gt_counts) if round(p) == round(g))
        
        # Find worst cases
        sorted_errors = sorted(zip(all_image_names, all_gt_counts, all_pred_counts, all_errors, all_rel_errors), 
        ...
        results[split] = {
            "overall": {
                "mae": mae,
                "rmse": rmse,
                "over_count_pct": over_count / len(all_gt_counts) * 100,
                "under_count_pct": under_count / len(all_gt_counts) * 100,
                "exact_count_pct": exact_count / len(all_gt_counts) * 100
            },
            "range_stats": range_stats,
            "worst_cases": sorted_errors[:20],  # Top 20 worst predictions
            "raw_data": {
                "gt": all_gt_counts,
                "pred": all_pred_counts,
                "errors": all_errors,
        ...
        print(f"Over-counting: {over_count}/{len(all_gt_counts)} images ({over_count/len(all_gt_counts)*100:.1f}%)")
        print(f"Under-counting: {under_count}/{len(all_gt_counts)} images ({under_count/len(all_gt_counts)*100:.1f}%)")
        print(f"Exact count: {exact_count}/{len(all_gt_counts)} images ({exact_count/len(all_gt_counts)*100:.1f}%)")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Error Analysis', parents=[get_argparser()])
